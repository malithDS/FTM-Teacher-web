<?php
	include('../inc/conn.php');

	// session_start();
	// if (!isset($_SESSION['nova_student_id']) ||(trim ($_SESSION['nova_student_id']) == '')) {
	// header('location:student_index.php');
    // exit();
	// }
	
	// $uquery=mysqli_query($conn,"SELECT nova_student_id,notification_key,index_no,name,email,nic,mobile_1,username,password
	//  FROM `nova_student` WHERE nova_student_id='".$_SESSION['nova_student_id']."'");
	// $urow=mysqli_fetch_assoc($uquery);
?>
<!DOCTYPE html>
<html>
<head>
<title>Lecturer student</title>
<script src="../js/jquery-3.1.1.js"></script>	
<script type="text/javascript">

$(document).keypress(function(e){ //using keyboard enter key
	displayResult();
	/* Send Message	*/	
	
		if(e.which === 13){ 
				if($('#msg').val() == ""){
				alert('Please write message first');
			}else{
				$msg = $('#msg').val();
				$id = $('#id').val();
				$.ajax({
					type: "POST",
					url: "send_message.php",
					data: {
						msg: $msg,
						id: $id,
					},
					success: function(){
						displayResult();
						$('#msg').val(''); //clears the textarea after submit
					}
				});
			}	

		} 
	}
); 


$(document).ready(function(){ //using send button
	displayResult();
	/* Send Message	*/	
		
		$('#send_msg').on('click', function(){
			if($('#msg').val() == ""){
				alert('Please write message first');
			}else{
				$msg = $('#msg').val();
				$id = $('#id').val();
				$.ajax({
					type: "POST",
					url: "Lecturer_send_message.php",
					data: {
						msg: $msg,
						id: $id,
					},
					success: function(){
						displayResult();
						$('#msg').val(''); //clears the textarea after submit
					}
				});
			}	
		});
	/* END */
	});
	
	function displayResult(){
		$id = $('#id').val();
		$.ajax({
			url: 'Lecturer_send_message.php',
			type: 'POST',
			async: false,
			data:{
				id: $id,
				res: 1,
			},
			success: function(response){
				$('#result').html(response);
			}
		});
	}
</script>

</head>

<body>
<table id="chat_room" style="align:center">
	<tr>
	<th><h4>Hi, <?php //echo $urow['username']; ?></a>  </h4></th>
	</tr>
	<?php
		$query=mysqli_query($conn,"select chat_with_lecturer_id,nova_sub_class_id,nova_student_id,message,date,time from `chat_with_lecturer`");
		$row=mysqli_fetch_array($query);
	?>
			<tr>
				<td>
				<div id="result" style="overflow-y:scroll; height:300px; width: 605px;"></div>
				<form class="form">
					<!--<input type="text" id="msg">--><br/>
					<textarea id="msg" rows="4" cols="85"></textarea><br/>
					<input type="hidden" value="<?php echo $row['nova_sub_class_id']; ?>" id="id">
					<button type="button" id="send_msg" class="button button2">Send</button>
				</form>
				</td>
			</tr>

</table>

</body>
</html>